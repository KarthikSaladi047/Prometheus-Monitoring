...
- Create a cron job that runs every 30 minutes. It creates a temporary file using mktemp, then executes the Python script push_register.py using Python 3, and discards any output or error messages generated by the script.
```
crontab -e
2
--
*/30 * * * * /bin/mktemp /tmp/push-XXXXX && /usr/bin/python3 /home/Pushgateway/push_register.py > /dev/null 2>&1
```
